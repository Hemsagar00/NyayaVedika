...
// Export as Word-compatible .docx (same HTML format, .docx extension for MS Word)
export function exportAsDocx(content, filename) {
  const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
export function exportDocument(content, format, filename) {
  ...
  switch (format) {
    case 'docx':
      return exportAsDocx(content, baseName);
    case 'doc':
